...
async function main(){
    ...
    while(true){
        ...
        if(obj?.element){
          ...
          let market:string = data.data.data.market.split("_")[0].toLowerCase()
            if(!obj){
            }else{
                if(data.data.type==="TRADE_ADDED"){
                    ...
                    const values=[timestamp,price,quantity,`${market.toUpperCase()}`]
                    console.log(values)
                    ...
                    const query=`update into ${market}_prices(time,price) values($1,$2)`;
  ...
  const { rows } = await pgClient.query(klineQuery,[market.toUpperCase()]);
  console.log("Rows",rows)

  if (rows.length > 0) {
    await redisClient.publish(
      `trade@${market.toUpperCase()}_USDC`,
      JSON.stringify({
        stream: `trade@${market}_USDC`,
        data: {
          e: "trade",
