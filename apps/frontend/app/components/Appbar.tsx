...
export const Appbar: React.FC = () => {
  ...
  return (
    <div className="text-white border-b border-slate-800 py-3 z-10">
      <div className="flex justify-between items-center p-2">
        ...
        <div className="flex justify-between w-[700px]">
          <span 
            className={`text-sm pt-1 flex justify-between items-center w-[100px] pl-8 cursor-pointer ${
              route.startsWith('/trade') ? 'text-white' : 'text-slate-500'
            }`} 
            onClick={() => router.push('/trade/SOL_USDC')}
          >
          ...
          <span className="flex flex-col items-center rounded-lg relative">
            <span className="flex bg-[#323546] p-2 rounded">
              ...
              <input 
                type="text" 
                className="bg-inherit text-sm outline-none ml-4" 
                placeholder="Search" 
            ...
            {showMarkets && (
              <ul className="w-full mt-10 absolute bg-[#323546] p-2 rounded cursor-pointer duration-500">
                <li 
                  className="text-xs hover:bg-gray-400 p-2 duration-300 rounded" 
                  onClick={handleMarketItemClick}
                >
                  SOL/USDC
                </li>
                <span className="text-slate-500 text-xs">
                  For now only SOL market is available
                </span>
              </ul>
            )}
          </span>
    ...
    <div className="z-[200] duration-200 absolute top-10 right-5 w-80 bg-[#1e1e2f] text-white rounded-xl p-4 space-y-4 font-sans">
      <div className="flex justify-between items-center">
        {user?.token ? (
          ...
          <>
            <button className="border border-blue-600 text-blue-500 px-4 py-1.5 rounded-md text-sm font-medium">
              Sign Up
            </button>
