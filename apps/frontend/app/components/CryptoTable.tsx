...
import { ChevronDown, Eye, Flame, Filter, LayoutGrid, PlusCircle, Signal } from "lucide-react";
...
<TopBar />
      ...
      <table className="min-w-full text-sm">
        ...
        <tbody>
          {data.map((coin) => (
            <tr key={coin.rank} className="border-b border-gray-800 cursor-pointer">
              <td className="py-7 "><span>{coin.rank}</span></td>
              

              <td className="py-7 font-semibold flex w-[100px] justify-between">
              <Image src={coin.image} alt="alt" width={20} height={20} className="mr-2"/> 
...
const TopBar = () => {
  ...
  const leftButtons = [
    { label: 'Coins', icon: <LayoutGrid size={16} />, className: 'bg-gray-800 text-white border border-blue-500' },
    { label: 'DexScan', icon: <LayoutGrid size={16} />, className: 'bg-gray-800 text-white' },
    { label: '📊 Top', icon: null, className: 'bg-blue-700 text-white' },
    { label: 'Trending', icon: <Flame size={16} />, className: 'text-orange-400' },
    { label: 'New', icon: <PlusCircle size={16} />, className: 'text-purple-400' },
    { label: 'Gainers', icon: <Signal size={16} />, className: 'text-green-400' },
    { label: 'Most Visited', icon: <Eye size={16} />, className: 'text-blue-400' },
  ];

  const rightButtons = [
    { label: 'Filters', icon: <Filter size={16} />, onClick: () => setShowFilter(true) },
    { label: 'Columns', icon: <LayoutGrid size={16} />, onClick: () => setShowColumn(true) },
  ...
  return (
    <div className="w-full flex flex-wrap items-center justify-between bg-gray-900 rounded-lg mb-4">
      ...
      {showFilter && (
        <>
        <div className="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm z-10" onClick={() => setShowFilter(false)}></div>
        <FilterSidebar showFilter={showFilter} setShowFilter={setShowFilter}/>
        </>
)}
{showColumn&&(
  <>
  <div className="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm z-10" onClick={() => setShowColumn(false)}></div>
<MetricsSelector showColumn={showColumn} setShowColumn={setShowColumn}/>
  </>
)}
    </div>
  );
